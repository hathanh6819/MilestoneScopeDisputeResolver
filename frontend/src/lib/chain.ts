...
import { studionet as sdkChain } from 'genlayer-js/chains';
...
export const readClient = createClient({ chain: sdkChain });
...
export async function loadSnapshot(): Promise<Snapshot> {
  const protocol = await read<{ name: string; version: number }>('get_protocol');
  ...
  const counts = await read<{ agreement_count: number }>('get_counts');
  if (!Number.isSafeInteger(counts.agreement_count) || counts.agreement_count < 0) throw new Error('Invalid agreement count');
  const accounting = await read<AccountingRecord>('get_accounting');
  ...
  // Explicit bounded directory, never fabricate older records.
  for (let id = Math.max(1, counts.agreement_count - 99); id <= counts.agreement_count; id++) {
    ...
    result.clauses[id] = await Promise.all(Array.from({ length: dispute.clause_count }, (_, index) => read<ClauseResultRecord>('get_clause_result', [id, index])));
  }
  return result;
}
